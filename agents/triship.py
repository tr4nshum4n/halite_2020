        ...
        if ship.next_action == None:
            ...
            if ship_states[ship.id] == "COLLECT":
                ...
                if ship.cell.halite < 100:
                    ...
                    if ship.position + directions[best].to_point() not in occ_points:
                        ship.next_action = directions[best]
            if ship_states[ship.id] == "DEPOSIT":
                # Move towards shipyard to deposit cargo
                direction = getDirTo(ship.position, me.shipyards[0].position, size)
                if direction: ship.next_action = direction
      #  if ship.next_action != None :#AND (ship.next_action != ShipAction.CONVERT):
 #       if ship.next_action != ShipAction.CONVERT:
#            occ_points.append(ship.position + ship.next_action)
      #  else:
      #      occ_points.append(ship.position)
